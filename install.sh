...
# Altera shell para usuários com home em /home
for user in $(awk -F: '$6 ~ /^\/home/ { print $1 }' /etc/passwd); do
    chsh -s /bin/zsh "$user"
done

# Também altera para root
chsh -s /bin/zsh root
...
echo "Criando .zshrc para todos os usuários..."

# Cria para usuários comuns
for user_home in /home/*; do
    if [ -d "$user_home" ]; then
        echo "$ZSHRC_CONTENT" > "$user_home/.zshrc"
        chown $(basename "$user_home"):$(basename "$user_home") "$user_home/.zshrc"
    fi
done
...
if [ -f kali-dark-theme.tar ]; then
    tar -xvf kali-dark-theme.tar
    mv -f usr/share/themes/Kali-Dark /usr/share/themes 2>/dev/null
fi
